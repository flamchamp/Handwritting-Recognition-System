...
//========================================================================
// Evalutaion program for HRSBinarySearch.
...
{
  std::cout
      << "usage: ./hrs-binary-search-eval [<train_size>] [<test_size>] [<linear_size>]"
      << std::endl
      << std::endl
      << "Evaluation program for HRSBinarySearch. You must use "
      << "full training set to get the accuracy! "
      << "Full training set size and full testing set size"
      << "will be used if no arguments are specified." << std::endl
      << std::endl
...
const int         full_training_size  = 60000;
const int         full_testing_size   = 10000;
const int         width               = 22;
const int         default_linear_size = 1000;
...
int main( int argc, char** argv )
{
  ...
  if ( argc != 3 && argc != 1 && argc != 4 ) {
    std::cout << "Invalid command line arguments!" << std::endl << std::endl;
    print_help();
    return 1;
  }
  ...
  if ( argc == 1 ) {
    ...
    if ( testing_size < 1 || testing_size > full_testing_size ) {
      std::cout << "Invalid testing size: " << testing_size << std::endl
                << std::endl;
      return 1;
    }
    ...
    if ( training_size < 1 || training_size > full_training_size ) {
      std::cout << "Invalid training size: " << training_size << std::endl
                << std::endl;
      return 1;
    }
  }
  ...
  std::cout << "Evaluating HRSBinarySearch..." << std::endl;
  std::cout << std::setw( width ) << std::left << " - training size"
            << " : " << training_size << std::endl;
  std::cout << std::setw( width ) << std::left << " - testing  size"
            << " : " << testing_size << std::endl;
  ...
  HRSBinarySearch clf( linear_size );
  ...
  std::cout << std::setw( width ) << std::left << " - training time"
            << " : " << training_time << " seconds" << std::endl;
  std::cout << std::setw( width ) << std::left << " - classification time"
            << " : " << classification_time << " seconds" << std::endl;

  // Report accuracy only if using the full traininig dataset

  if ( training_size == full_training_size &&
       testing_size == full_testing_size )
    std::cout << std::setw( width ) << std::left << " - accuracy"
              << " : " << accuracy << std::endl;
